...
from .utils import q_search_checks, q_search_users
...
class UsersListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if q:
            qs = qs.filter(q_search_users(q))

        return qs

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['titles'] = {"ID": "id", "Логин": "username", "Email": "email", "Имя": "first_name", "Фамилия": "last_name", "Отчество": "profile__middle_name", "Бесплатные проверки": "profile__free_checks", "Подписка": "profile__subscribed"}
...
class ChecksListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if q:
            qs = qs.filter(q_search_checks(q))

        return qs

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['titles'] = {"ID": "id", "Название": "title", "Текст": "body", "Создана": "created", "Фейк": "is_fake", "Пользователь": "user"}
